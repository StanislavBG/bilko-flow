...
export interface ColumnDefinition {
  /** Column name (e.g. "user_id", "email"). */
  name: string;
  /** SQL/domain type (e.g. "text", "integer", "serial", "timestamp"). */
  type: string;
  ...
  description?: string;
  /** Default value expression (e.g. "now()", "0", "'active'"). */
  defaultValue?: string;
  /** Whether this column has a UNIQUE constraint. */
  unique?: boolean;
  /** Foreign key reference in the format "table_name.column_name". */
  references?: string;
}

/** A table definition within a schema. */
export interface TableDefinition {
  /** Unique table identifier (used as the key). */
  id: string;
  /** Table name (e.g. "users", "projects"). */
  name: string;
...
/** A complete schema containing multiple tables. */
export interface SchemaDefinition {
  /** Unique schema identifier. */
  id: string;
  /** Schema name (e.g. "bilko-schema", "public"). */
  name: string;
...
/** Props for the SchemaDesigner component. */
export interface SchemaDesignerProps {
  /** The schema to display and edit. */
  schema: SchemaDefinition;
  /** Called when the schema is mutated (table added, removed, modified). */
  onSchemaChange: (schema: SchemaDefinition) => void;
  /**
   * Called when the user submits text/file content for LLM-based table creation.
   * The consumer provides the LLM integration — the component only handles the UI.
   * Returns a TableDefinition parsed from the text, or throws on failure.
   */
  onParseTableFromText?: (text: string) => Promise<TableDefinition>;
  /** Additional CSS classes on root element. */
  className?: string;
}

/** Props for the SmartTableCreator widget. */
export interface SmartTableCreatorProps {
  /** Existing table IDs (used to prevent duplicate names). */
  existingTableIds: string[];
  /** Called when a new table definition is created. */
  onTableCreated: (table: TableDefinition) => void;
  /** Called to close/dismiss the creator widget. */
  onCancel: () => void;
  /**
   * Called when the user submits text for LLM-based table creation.
   * If not provided, only manual table creation is available.
   */
  onParseTableFromText?: (text: string) => Promise<TableDefinition>;
}
